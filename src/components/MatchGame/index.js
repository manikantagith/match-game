...
class MatchGame extends Component {
  ...
  renderGame = () => {
    const {activeMatch, activeTabId} = this.state
    const {imageUrl} = activeMatch
    const filteredImages = this.filterImagesList()
    return (
  ...
  render() {
    const {timeRemaining, score, isGameOver} = this.state
    ...
    return (
      <div className="app-container">
        ...
        <div className="match-game-container">
          {isGameOver ? this.renderScore() : this.renderGame()}
        </div>
      </div>
    )
  }
